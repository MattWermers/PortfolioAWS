...
const submissionResultIcon = document.getElementById('submissionResultIcon');
const submissionResultMsg  = document.getElementById('submissionResultMsg');
...
resetBtn.addEventListener('click', () => {
  ...
  landingSection.style.display = '';   // restore to CSS default (flex)
  uploadsDiv.style.display    = '';   // restore to CSS default (none) — user must click New Job again
  statsPanel.style.display = 'none';
  toolbar.style.display = 'none';
  workspace.style.display = 'none';
  exportPanel.style.display = 'none';
  submissionResult.style.display = 'none';
  submissionResultIcon.textContent = '';
  submissionResultMsg.textContent = '';
  fileInput.value = '';
...
function renderExpandedTable() {
  ...
  // Data rows — rows with ANY field error are excluded from the final preview
  // (they will not be importable and are already visible in the mapping table)
...
function showSubmissionResult(success, message) {
  ...
  submissionResultIcon.textContent = success ? '✓' : '✗';
  submissionResultIcon.className   = success ? 'result-icon result-ok' : 'result-icon result-fail';
  submissionResultMsg.textContent  = message;
  submissionResultMsg.className    = success ? 'result-msg result-ok' : 'result-msg result-fail';
...
if (submitBackendBtn) {
  submitBackendBtn.addEventListener('click', async () => {
    ...
    // Chunk the processed rows
    const CHUNK_SIZE = 500;
    const processedRows = allDataRows.map((row, idx) => {
      const processed = ImportPipeline.processRow(row, headers, mapping, fieldFormats, fieldFormatInclusive.map(v => !v), idx);
      return {
        rowIndex:   idx,
    ...
    const chunks = ImportPipeline.chunkRows(processedRows, CHUNK_SIZE);
    ...
    try {
      ...
      if (!createRes.ok) {
        ...
        throw new Error(`Server rejected the job (${createRes.status}): ${errText}`);
      }
      ...
      for (let idx = 0; idx < totalChunks; idx++) {
        const chunkRes = await fetch(`${API_BASE_URL}/jobs/${jobId}/chunks`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ chunkIndex: idx, rows: chunks[idx] })
        });

        if (!chunkRes.ok) {
          const errText = await chunkRes.text();
          throw new Error(`Chunk ${idx + 1} rejected (${chunkRes.status}): ${errText}`);
        }
      }

      // 3. Report acceptance
      showSubmissionResult(true, `Job accepted — ID: ${jobId} · ${totalDataRows.toLocaleString()} rows across ${totalChunks} chunk(s) submitted.`);

    } catch (err) {
      showSubmissionResult(false, `Submission failed: ${err.message}`);
    } finally {
